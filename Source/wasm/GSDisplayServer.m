...
+ (void) initialize
{
	...
	if (serverLock == nil) {
		...
		if (serverLock == nil) {
			...
			windowmaps = NSCreateMapTable(NSNonOwnedPointerMapKeyCallBacks, NSNonOwnedPointerMapValueCallBacks, 20);
#ifndef GNUSTEP_NO_MULTI_THREAD
		}
 ...
 * be initialized. Typically these values are specific to the concrete
 * implementation. The current set of attributes that can be used with
 * GSDisplayServer is.
 * </p>
 * <list>
 ...
 * </list>
 * <p>
 * GSDisplayName is window server specific and shouldn't be used when creating
 * a GSDisplayServer (although you can retrieve the value with the -attributes method). 
 * On X-Windows the value might be set to something like "host:d.s" where host is 
 * the host name, d is the display number and \a s is the screen number. 
 * GSDisplayNumber indicates the number of the display to open. GSScreenNumber 
 * indicates the number of the screen to display on. If not explicitly set, these
 * attributes may be taked from environment variables or from other operating 
 * specific information.
 * </p>
 * <p>In almost all applications one would only create a single instance of a
 * window server. Although it is possible, it is unlikely that you would need
 * more than one window server (and you would have to be very careful how you 
 * handled window creation and events in this case).
 * </p>
 */
+ (GSDisplayServer *) serverWithAttributes: (NSDictionary *)attributes
{
	...
	if (self == [GSDisplayServer class]) {
		server = [[defaultServerClass allocWithZone: _globalGSZone] initWithAttributes: attributes];
	} else {
		server = [[self allocWithZone: _globalGSZone] initWithAttributes: attributes];
	}
 ...
 * etc. This method must be called after a window server is created in order to 
 * make it available to the rest of the GUI library
 */
...
/** 
 * <init />
 * Initializes the server. This typically causes the receiver to  <em>connect</em>
 * to the display (e.g. XOpenDisplay () on an X-Windows server). 
 */
- (id) initWithAttributes: (NSDictionary *)attributes
{
	...
	event_queue = [[NSMutableArray allocWithZone: [self zone]] initWithCapacity: 32];
	drag_types = NSCreateMapTable(NSIntMapKeyCallBacks, NSObjectMapValueCallBacks, 0);
...
/**
 * Causes the server to disconnect from the display. If the receiver is the current
 * server, it removes itself and sets the current  server to nil. Sending any more
 * messages to the receiver after this is likely to cause severe problems and 
 * probably crash the application. 
 */
- (void) closeServer
{
...
/** 
 * Returns YES if the backend handles window decorations and NO if the gui library
 ...
 * connection to the display. On X-Windows, for example, this is a pointer to the
...
// 
// MARK: GNUstep Window operations */
//
...
/**
 * Creates a window whose location and size is described by frame and whose 
 * backing store is described by type. This window is not mapped to the screen
 * by this call.<br />
 * 
 * Note that frame is the frame of the entire GNUstep window including borders,
 * titlebar and other standard decorations.<br />
 * If -handlesWindowDecorations returns YES, the backend will produce (and return
 * the identifier of) a smaller drawable window inside this decorated area.<br/>
 * Use -styleoffsets::::: to determine the extent of the decorations and determine
 ...
 * ordered front to back.  The front most window being the first object in the array.  
 * The default implementation returns the visible windows in an unspecified order.
 */
- (NSArray *) windowlist
{
 ...
 * irected only to the window win. If successful, the return value
 * is YES and this message must be balanced by a `-releasemouse` message.
 */
...
/** 
 * Create a standard cursor (such as an arrow or I-beam). Returns a
...
// 
// MARK: GNUstep Event Operations
// 

@implementation GSDisplayServer (EventOps)

static NSMutableArray *spare_event_queue = nil;
...
- (NSEvent*) getEventMatchingMask: (unsigned)mask beforeDate: (NSDate*)limit inMode: (NSString*)mode dequeue: (BOOL)flag
{
	...
	if (spare_event_queue == nil) {
		spare_event_queue = [[NSMutableArray allocWithZone: [self zone]] initWithCapacity: 32];
	} else {
	...
	do {
		...
		if (count == 0) {
			...
			// Special case - if the mask matches any event, we just get the first event on the queue.
...
- (void) discardEventsMatchingMask: (unsigned)mask beforeEvent: (NSEvent*)limit
{
	...
	while (index-- > 0) {
		...
		if ([event timestamp] < when) {
			...
			if ((mask == NSAnyEventMask) || (mask & NSEventMaskFromType([event type]))) {
...
/** 
 * Posts an event to the event queue. The value of flag determines whether the 
 * event is inserted at the start of the queue or appended at the end.
 */
- (void) postEvent: (NSEvent*)anEvent atStart: (BOOL)flag
{
	//fprintf(stderr, "entering call at %s\n", __PRETTY_FUNCTION__);
	if (flag) {
