...
static NSString	*GSHideOtherApplicationsNotification = @"GSHideOtherApplicationsNotification";
static NSString	*GSUnhideAllApplicationsNotification = @"GSUnhideAllApplicationsNotification";
 ...
 */
static void _NSAppKitUncaughtExceptionHandler (NSException *exception)
{
	NSInteger retVal;

	// Reset the exception handler to the Base library's one, to prevent recursive calls to the gui one.
	NSSetUncaughtExceptionHandler(defaultUncaughtExceptionHandler);  

	// If there is no graphics context to run the alert panel in or its a severe error,
	// use a non-graphical exception handler
	if (GSCurrentContext() == nil
		...
		// The following will raise again the exception using the base library exception handler
		[exception raise];
	}

#ifdef DEBUG
	retVal = GSRunExceptionPanel([NSString stringWithFormat: _(@"Critical Error in %@"), [[NSProcessInfo processInfo] processName]],
		exception, _(@"Abort"), nil, _(@"Debug"));
#else
	retVal = GSRunExceptionPanel([NSString stringWithFormat: _(@"Critical Error in %@"), [[NSProcessInfo processInfo] processName]],
		exception, _(@"Abort"), nil, nil);
#endif
...
NSBundle *GSGuiBundle(void)
{
	...
	if (!guiBundle) {
		...
		guiBundle = [NSBundle bundleForLibrary: @"gnustep-gui" version: OBJC_STRINGIFY(GNUSTEP_GUI_MAJOR_VERSION.GNUSTEP_GUI_MINOR_VERSION)];
		RETAIN(guiBundle);
	}
	return guiBundle;
}
...
static NSString *gnustep_backend_path(NSString *dir, NSString *name)
{
	NSString *path;
	NSEnumerator *benum;
...
/**
 * @private
 * Find and load the backend framework, if there is one. The name is taken from a user default containing
 * the name of the backend framework, such as 'GNUstep-back', or simply 'back', or for historical reasons,
 * 'libgnustep-back'.
 */
static NSString *gnustep_backend_framework(NSString *bundleName)
{
	if (bundleName == nil) {
		...
		if ([bundleName hasPrefix: @"GNUstep-"]) {
			...
			bundleName = [NSString stringWithFormat: @"GNUstep-%@.framework", bundleName];
		} 
	}
 ...
 * @private
 * 
 * Find and load the backend bundle, if there is one. The name is taken from a user default containing
 * the name of the backend bundle, such as 'back', or for historical reasons, 'libgnustep-back'. New
 * versions may also have a version number associated with it.
 */
static NSString *gnustep_backend_bundle(NSString *bundleName)
{
	...
	if (bundleName == nil) {
		...
		if ([bundleName hasPrefix: @"libgnustep-"] == NO) {
			bundleName = [NSString stringWithFormat: @"libgnustep-%@", bundleName];
		} 
	}
	
	bundleWithVersion = [NSString stringWithFormat: @"%@-%03d.bundle", bundleName, version];
	bundleName = [bundleName stringByAppendingString: @".bundle"];
	path = gnustep_backend_path(@"Bundles", bundleWithVersion);
	
	if (path == nil) {
		NSLog(@"Did not find correct version of backend (%@), falling back to std (%@).", bundleWithVersion, bundleName);
...
BOOL initialize_gnustep_backend(void)
{
	...
	if (first) {
		...
		// Remember which thread we are running in. This thread will be the -gui thread, ie. 
		// the only thread that may do any rendering. With the exception of a few methods
		// explicitly marked as thread-safe, other threads should not call any methods in -gui.
		...
		{      
			...
			NSCAssert1(theBundle != nil,  _(@"Can't create NSBundle object for backend at path %@"), path);

			// Now load the object file from the bundle.  
			NSCAssert1 ([theBundle load], _(@"Can't load object file from backend at path %@"), path);
			
			// Now get the GSBackend class, which should have just been loaded from the bundle.
			backend = NSClassFromString (@"GSBackend");
			NSCAssert1 (backend != Nil, _(@"Backend at path %@ doesn't contain the GSBackend class"), path);
...
- (id) _targetForAction: (SEL)aSelector keyWindow: (NSWindow *)keyWindow mainWindow: (NSWindow *)mainWindow;
...
{     
	if ([[NSUserDefaults standardUserDefaults] boolForKey: @"GSSuppressAppIcon"] == NO) {
...
{
	[super _initDefaults];
	// Set the title of the window to the process name. Even as the window shows no title bar, the window manager may show it.
...
#if	MINI_ICON
	// Hack ... 
	// At least one window manager won't miniaturize a window unless it's at the standard level.
	// If the app icon is suppressed, we may still want a miniaturised version while the app is hidden.
	if (YES == [[NSUserDefaults standardUserDefaults] boolForKey: @"GSSuppressAppIcon"]) {
		return;
	}
#endif
	// App icons and mini windows are displayed at dock level by default. Yet, with the current window level
	// mapping in -back, some window managers will order pop up and context menus behind app icons and 
	// mini windows. Therefore, it is possible to have app icons and mini windows displayed at normal window level 
	// under control of a user preference. See also NSMiniWindow -_initDefaults in NSWindow.m
...
- (void) rightMouseDown: (NSEvent *)theEvent
{
	...
	if (menu) {
		[NSMenu popUpContextMenu: menu withEvent: theEvent forView: [self contentView]];
	} else {
...
- (void) mouseDown: (NSEvent*)theEvent
{
	if ([theEvent clickCount] >= 2) {
		...
		NSPoint	location;
		NSUInteger eventMask = NSLeftMouseDownMask | NSLeftMouseUpMask | NSPeriodicMask | NSOtherMouseUpMask | NSRightMouseUpMask;
		...
		while (!done) {
			theEvent = [NSApp nextEventMatchingMask: eventMask untilDate: theDistantFuture inMode: NSEventTrackingRunLoopMode dequeue: YES];
...
+ (void) detachDrawingThread: (SEL)selector toTarget: (id)target withObject: (id)argument
{
	...
	inv = [NSInvocation invocationWithMethodSignature:  [target methodSignatureForSelector: selector]]; 
	[inv setTarget: target];
	[inv setSelector: selector];
	[inv setArgument: argument atIndex: 2];
	[NSThread detachNewThreadSelector: @selector(_invokeWithAutoreleasePool:)  toTarget: self withObject: inv];
}
...
- (void) _init
{
	...
	// we refer or use in this method) might be calling [NSApplication sharedApplication]
	// during their initialization, and we want those calls to succeed.  
	NSApp = self;
...
- (id) init
{
	...
	// The appkit should run in the main thread ... so to be sure we perform all the initialisation there.
#ifndef GNUSTEP_NO_MULTI_THREAD
	[self performSelectorOnMainThread: @selector(_init) withObject: self waitUntilDone: YES];
#else
	[self _init];
#endif
...
- (void) finishLaunching
{
	...
	// the windows were created and set to be key/main earlier - before the app was active.
	...
	// If there was more than one window set as key or main, we must make sure that 
	// the one we have recorded is the real one by making it become key/main again.
	if (hadDuplicates) {
	...
	[nc addObserver: self selector: @selector(_windowWillClose:) name: NSWindowWillCloseNotification object: nil];
	[nc addObserver: self selector: @selector(_windowDidBecomeKey:) name: NSWindowDidBecomeKeyNotification object: nil];
	[nc addObserver: self selector: @selector(_windowDidBecomeMain:) name: NSWindowDidBecomeMainNotification object: nil];
	[nc addObserver: self selector: @selector(_windowDidResignKey:) name: NSWindowDidResignKeyNotification object: nil];
	[nc addObserver: self selector: @selector(_windowDidResignMain:) name: NSWindowDidResignMainNotification object: nil];

	// register as observer for hide/unhide notifications 
	[[[NSWorkspace sharedWorkspace] notificationCenter]
		addObserver: self selector: @selector(_workspaceNotification:) name: GSHideOtherApplicationsNotification object: nil];
	[[[NSWorkspace sharedWorkspace] notificationCenter]
		addObserver: self selector: @selector(_workspaceNotification:) name: GSUnhideAllApplicationsNotification object: nil];
	...
	// Instantiate the NSDocumentController if we are a doc-based app and eventually reopen all autosaved documents
	...
	if ((files = [self _openFiles]) != nil) {
		...
		BOOL docBased = [[sdc documentClassNames] count] > 0 && [sdc defaultType] != nil;
		BOOL shouldOpen = docBased ? YES : NO;

		if ([_delegate respondsToSelector: @selector(applicationShouldOpenUntitledFile:)]) {
		...
		if (shouldOpen) {
			...
			} else if ([_delegate respondsToSelector: @selector(applicationOpenUntitledFile:)]) {
...
/*
 * Posts \c NSApplicationDidFinishLaunchingNotification.
 *
 * <p>The -run method calls this the first time it is called, before starting
...
- (void) _didFinishLaunching
{
	...
	@try {
		[[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: NSWorkspaceDidLaunchApplicationNotification
				object: [NSWorkspace sharedWorkspace]
				userInfo: [self _notificationUserInfo]];
	} @catch (NSException * localException) {
		NSLog(_(@"Problem during launch app notification: %@"), [localException reason]);
...
- (void) activateIgnoringOtherApps: (BOOL)flag
{
	...
	if (_app_is_active == NO) {
		...
		[nc postNotificationName: NSApplicationWillBecomeActiveNotification object: self];

		_app_is_active = YES;

		if ([[NSUserDefaults standardUserDefaults] boolForKey: @"GSSuppressAppIcon"]) {
			[_app_icon_window orderOut: self];
		}

		// Make sure to calculate count after the notification, since inactive status might
		// be changed by a notifiee. 
		...
		if ([self keyWindow] == nil && _hidden_key != nil && [[self windows] indexOfObjectIdenticalTo: _hidden_key] != NSNotFound) {
			[_hidden_key makeKeyWindow];
			_hidden_key = nil;
		}

		if ([self mainWindow] == nil && _hidden_main != nil && [[self windows] indexOfObjectIdenticalTo: _hidden_main] != NSNotFound) {
		...
		if ([self keyWindow] != nil) {
			...
			// We need give input focus to some window otherwise we'll never get keyboard events. FIXME: doesn't work.
			NSWindow *menu_window= [[self mainMenu] window];
			NSDebugLLog(@"Focus", @"No key on activation - make menu key");
			[GSServerForWindow(menu_window) setinputfocus: [menu_window windowNumber]];
		}

		info = [self _notificationUserInfo];
		[nc postNotificationName: NSApplicationDidBecomeActiveNotification object: self userInfo: info];
		[[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: NSApplicationDidBecomeActiveNotification
...
- (void) deactivate
{
	...
	if (_app_is_active == YES) {
		...
		[nc postNotificationName: NSApplicationWillResignActiveNotification object: self];
		...
		info = [self _notificationUserInfo];
		[nc postNotificationName: NSApplicationDidResignActiveNotification object: self userInfo: info];
		[[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: NSApplicationDidResignActiveNotification object: [NSWorkspace sharedWorkspace] userInfo: info];
	}
}
...
{
	[[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: GSHideOtherApplicationsNotification object: [NSWorkspace sharedWorkspace] userInfo: [self _notificationUserInfo]];
}
...
{
	[[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: GSUnhideAllApplicationsNotification object: [NSWorkspace sharedWorkspace] userInfo: [self _notificationUserInfo]];
}
...
- (void) _handleException: (NSException *)exception
{
	...
	// If we are in debug mode, then rethrow the exception so that the application can be stopped.
...
// MARK: Running the main event loop

void _gstep_fire_runLoop() {
	NSApplication *app = NSApp;
	struct _NSModalSession *currentSession = app->_session;
	if (currentSession != 0) {

		@try {

			NSInteger status = [NSApp runModalSession: currentSession];

			if (status != NSRunContinuesResponse) {
				[app endModalSession: currentSession];
			}
			...
			if (currentSession != 0) {
				...
				NSWindow *win_to_close = currentSession->window;
...
{
	NSEvent *e;
	id distantFuture = [NSDate distantPast];     // Cache this, safe 

	if (_runLoopPool != nil) {
		[NSException raise: NSInternalInconsistencyException format: @"NSApp's run called recursively"];
	}

	// Set this flag here in case the application is actually terminated inside -finishLaunching.
	_app_is_running = YES;
	...
	// building a cache of events which will be firted in this interation.
	int len = -1;
	int max = 32;
	NSEvent *queue[32];

	for(int i = 0; i < max;i++) {
		e = [self nextEventMatchingMask: NSAnyEventMask untilDate: distantFuture inMode: NSDefaultRunLoopMode dequeue: YES];
		if (e == nil) {
			len = i;
			break;
		}
...
#if 0
	// Every single non trivial line of code must be enclosed into an autorelease pool.
	// Create an autorelease pool here to wrap synchronize and the NSDebugLog.
	IF_NO_GC(_runLoopPool = [arpClass new]);

	[[NSUserDefaults standardUserDefaults] synchronize];
	//DESTROY (_runLoopPool);
	_wasmHost_enque_runloop();
#endif
}
...
// MARK: Running modal event loops
...
{
	NSModalSession newSession;

	newSession = (NSModalSession)NSZoneMalloc(NSDefaultMallocZone(), sizeof(struct _NSModalSession));
	newSession->runState = NSRunContinuesResponse;
	newSession->entryLevel = [theWindow level];
	newSession->window = theWindow;
	newSession->previous = _session;
	_session = newSession;

	// Displaying / raising window but centering panel only if not up seems to match the behavior on OS X (Panther).
...
- (void) endModalSession: (NSModalSession)theSession
{
	...
	if (theSession == 0) {
		[NSException raise: NSInvalidArgumentException format: @"null pointer passed to endModalSession:"];
	}
	...
	if (tmp == 0) {
		[NSException raise: NSInvalidArgumentException format: @"unknown session passed to endModalSession:"];
	}
	...
	if ((_session != 0) && ([windows indexOfObjectIdenticalTo: _session->window] != NSNotFound)) {
...
/**
 * Starts modal event loop for given window, after calling `-beginModalSessionForWindow:`.
 * The loop is broken only by `-stopModal`, `-stopModalWithCode:`, or `-abortModal`, at which
 * time `-endModalSession:` is called automatically.
 */
...
- (NSInteger) runModalSession: (NSModalSession)theSession
{
	...
	//fprintf(stderr, "will fire %i events at %s\n", len, __PRETTY_FUNCTION__);
	...
	for(int idx = 0; idx < len; idx++)
	{
		...
		if (event != nil) {
			...
			//fprintf(stderr, "eventWindow %p\n", eventWindow);
			if (eventWindow == theSession->window || [eventWindow worksWhenModal] == YES || [event type] == NSAppKitDefined) {
				ASSIGN(_current_event, event);
			} else {
				//fprintf(stderr, "did discard event\n");
		...
		if (event != nil) {
			...
			if ([[self windows] indexOfObjectIdenticalTo: _session->window] == NSNotFound || ![_session->window isVisible]) {
				[self stopModal];
			}
		}
		//[pool drain];
...
- (void) stop: (id)sender
{
	if (_session != 0) {
		...
		_app_is_running = NO;
		// add dummy event to queue to assure loop cycles at least one more time
...
{
	// According to the spec, there is no exception which is thrown if we are not currently in a modal session.
	// While it is not good practice to call this when we're not, we shouldn't throw an exception.
	if (_session != 0 && _session->runState == NSRunContinuesResponse) {
		
		if (returnCode == NSRunContinuesResponse) {
			[NSException raise: NSInvalidArgumentException format: @"stopModalWithCode: with NSRunContinuesResponse"];
		}
...
- (NSInteger) runModalForWindow: (NSWindow *)theWindow relativeToWindow: (NSWindow *)docWindow
{
	if ((docWindow != nil) && (theWindow != nil)) {
		...
		NSDebugLLog(@"NSWindow", @"Positioning window %@ relative to %@ at %@", NSStringFromRect(theFrame), NSStringFromRect(docFrame), NSStringFromPoint(point));
		// Position window...
		[theWindow setFrameOrigin: point];
	}
	[theWindow orderWindow: NSWindowAbove relativeTo: [docWindow windowNumber]];
...
- (void) beginSheet: (NSWindow *)sheet modalForWindow: (NSWindow *)docWindow modalDelegate: (id)modalDelegate didEndSelector: (SEL)didEndSelector contextInfo: (void *)contextInfo
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName: NSWindowWillBeginSheetNotification object: docWindow];
	ret = [self runModalForWindow: sheet  relativeToWindow: docWindow];

	if (modalDelegate && [modalDelegate respondsToSelector: didEndSelector]) {
		void (*didEnd)(id, SEL, id, NSInteger, void*);

		didEnd = (void (*)(id, SEL, id, NSInteger, void*))[modalDelegate methodForSelector:  didEndSelector];
	...
	[[NSNotificationCenter defaultCenter]  postNotificationName: NSWindowDidEndSheetNotification object: docWindow];
}
...
- (void) sendEvent: (NSEvent *)theEvent
{
	...
	switch (type) {
		...
		case NSKeyDown:
		{
			...
			// Key equivalents must be looked up explicitly in the Services menu after checking the main menu, 
			// as NSMenu's -performKeyEquivalent: ignores the Services menu. See the comment in that method for
			// a rationale.
		...
		default:	// pass all other events to the event's window	
		{
			...
			if (type == NSMouseMoved) {
				NSDebugLLog(@"NSMotionEvent", @"Send move (%d) to %@", (int)type, window);
			} else {
				NSDebugLLog(@"NSEvent", @"Send NSEvent type: %@ to %@",  theEvent, window);
			}
			if (window) {
...
IF_NO_GC(NSAssert([event retainCount] > 0, NSInternalInconsistencyException));
		// If we are not in a tracking loop, we may want to unhide a hidden because the mouse has been moved.
		if (mode != NSEventTrackingRunLoopMode) {
			...
			if ([NSCursor isHiddenUntilMouseMoves]) {
				...
				if ((type == NSLeftMouseDown) || (type == NSLeftMouseUp) || (type == NSOtherMouseDown) || (type == NSOtherMouseUp)
...
- (id) targetForAction: (SEL)theAction to: (id)theTarget from: (id)sender
{
	...
	if (theTarget) {
		...
		NSWindow *toolbarWindow = [[[(NSToolbarItem *)sender toolbar] _toolbarView] window];
		...
		return [self _targetForAction: theAction keyWindow: keyWindow mainWindow: toolbarWindow];
...
- (id) targetForAction: (SEL)aSelector
{
	...
	return [self _targetForAction: aSelector keyWindow: [self keyWindow] mainWindow: mainWindow];
}
...
- (void) setApplicationIconImage: (NSImage*)anImage
{
	...
	if (imageSize.width > miniWindowSize.width || imageSize.height > miniWindowSize.height) {
...
- (void) hide: (id)sender
{
	...
	if (_app_is_hidden == NO) {
		...
		if (![[NSUserDefaults standardUserDefaults] boolForKey: @"GSSuppressAppIcon"]) {
			...
			// On hiding we also deactivate the application which will make the menus go away too.
			[self deactivate];
			_unhide_on_activation = YES;
			
			info = [self _notificationUserInfo];
			[nc postNotificationName: NSApplicationDidHideNotification object: self userInfo: info];
			[[[NSWorkspace sharedWorkspace] notificationCenter]
				postNotificationName: NSApplicationDidHideNotification object: [NSWorkspace sharedWorkspace] userInfo: info];
...
- (void) unhideWithoutActivation
{
	if (_app_is_hidden == YES) {
		...
		info = [self _notificationUserInfo];
		[nc postNotificationName: NSApplicationDidUnhideNotification object: self userInfo: info];
		[[[NSWorkspace sharedWorkspace] notificationCenter]
			postNotificationName: NSApplicationDidUnhideNotification object: [NSWorkspace sharedWorkspace] userInfo: info];
	}
}
...
- (void) arrangeInFront: (id)sender
{
	...
	if (menu) {
		...
		for (i = 0; i < count; i++) {
			...
			if ([win isKindOfClass: [NSWindow class]] && [win isVisible] && ![win isMiniaturized]) {
...
- (void) orderFrontStandardInfoPanelWithOptions: (NSDictionary *)dictionary
{
	...
	[_infoPanel setTitle: NSLocalizedString (@"Info", @"Title of the Info Panel")];
	[_infoPanel orderFront: self];
}

// MARK: Getting the main menu
...
- (void) rightMouseDown: (NSEvent*)theEvent
{
	...
	if (_main_menu != nil) {
		[NSMenu popUpContextMenu: _main_menu withEvent: theEvent forView: nil];
	} else {
		[super rightMouseDown: theEvent];
	}
}
...
// MARK: Managing the Windows menu
 ...
 * \a aString is assumed to be a filename representation the way 
...
- (void) changeWindowsItem: (NSWindow*)aWindow title: (NSString*)aString filename: (BOOL)isFilename
{
	...
	if (![aWindow isKindOfClass: [NSWindow class]]) {
		[NSException raise: NSInvalidArgumentException format: @"Object of bad type passed as window"];
	}
	...
	for (i = 0; i < count; i++) {
		...
		if ([item target] == aWindow) {
			...
			// If our menu item already exists and with the correct title, we need not continue.  
			if ([[item title] isEqualToString: aString]) {
				return;
			} else {
				// Else, we need to remove the old item and add it again with the new title.
				// Then new item might be located somewhere else in the menu than the old one
				// (because items in the menu are sorted by title) ... this is why we remove 
				// the old one and then insert it again.
	...
	// Can't permit an untitled window in the window menu... so if the window has not title, we don't add it to the menu.
	...
	if (count > 0 && sel_isEqual([[itemArray objectAtIndex: count-1] action], @selector(performClose:))) {
		count--;
	}

	if (count > 0 && sel_isEqual([[itemArray objectAtIndex: count-1] action], @selector(performMiniaturize:))) {
		count--;
	}

	for (i = 0; i < count; i++) {
		NSMenuItem *item = [itemArray objectAtIndex: i];
		if ([[item target] isKindOfClass: [NSWindow class]] && sel_isEqual([item action], @selector(makeKeyAndOrderFront:))) {
			break;
		}
	}
	...
	item = [_windows_menu insertItemWithTitle: aString action: @selector(makeKeyAndOrderFront:) keyEquivalent: @"" atIndex: i];
...
/**
 * Update Windows menu item for aWindow, to reflect its edited status. 
 * This is usually done automatically so you don't need to call this.
 */
- (void) updateWindowsItem: (NSWindow*)aWindow
{
	...
	if (menu != nil) {
		...
		if (found == NO) {
			[self changeWindowsItem: aWindow title: [aWindow title] filename: [aWindow _hasTitleWithRepresentedFilename]];
		}
	}
}
...
- (void) setWindowsMenu: (NSMenu*)aMenu
{
	...
	
	{
		...
		for (i = 0; i < count; i++) {
			...
			if (([win isExcludedFromWindowsMenu] == NO) && ([win isVisible] || [win isMiniaturized])) {
				[self changeWindowsItem: win title: [win title] filename: [win _hasTitleWithRepresentedFilename]];
			}
		}
	}
}
...
{
	if (_delegate != nil && ![_delegate isKindOfClass: [NSResponder class]] && [_delegate respondsToSelector: @selector(validRequestorForSendType:returnType:)]) {
		return [_delegate validRequestorForSendType: sendType returnType: returnType];
	}
...
- (BOOL) presentError: (NSError *)error
{
	...
	if (result != NSAlertErrorReturn) {
		...
		result = 1 - result;
		return [[error recoveryAttempter] attemptRecoveryFromError: error optionIndex: result];
	} else {
		return NO;
	}
}
...
- (void) _didPresentError: (NSWindow*)sheet  returnCode: (NSInteger)result contextInfo: (void*)context
{
	struct _DelegateWrapper *wrapper;
	id delegate;
	SEL sel;
	...
	if (result != NSAlertErrorReturn) {
		// Convert result (1, 0, -1) into index (0, 1, 2)
		result = 1 - result;
		recover = [[error recoveryAttempter] attemptRecoveryFromError: error optionIndex: result];
	} else {
		recover = NO;
	}
...
- (void) terminate: (id)sender
{
	...
	sdc = [NSDocumentController sharedDocumentController];
	if ([[sdc documentClassNames] count] > 0) {
		if ([sdc reviewUnsavedDocumentsWithAlertTitle: _(@"Quit") cancellable: YES] == NO) {
			return;
		}
	}
...
- (void) replyToApplicationShouldTerminate: (BOOL)shouldTerminate
{
	...
	if (shouldTerminate && _app_is_running) {
		[nc postNotificationName: NSApplicationWillTerminateNotification object: self];
		...
		[[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: NSWorkspaceDidTerminateApplicationNotification object: [NSWorkspace sharedWorkspace] userInfo: [self _notificationUserInfo]];
...
- (void) _loadAppIconImage
{
	...
	if (image == nil) {
		...
		// must first check that any existing image of the same name has its name removed.
...
- (void) _appIconInit
{
	...
	suppress = [[NSUserDefaults standardUserDefaults] boolForKey: @"GSSuppressAppIcon"];
#if	MINI_ICON
	if (suppress) {
	...
	NSWindow *tmpIconWindow = [[NSIconWindow alloc] initWithContentRect: NSZeroRect styleMask: mask backing: NSBackingStoreRetained defer: NO screen: nil];
...

	{
		...
		iconContentRect = GSGetIconFrame(tmpIconWindow);
		iconFrame = [tmpIconWindow frameRectForContentRect: iconContentRect];
		...
		[tmpIconWindow setFrame: iconFrame display: YES];

		iv = [[NSAppIconView alloc] initWithFrame: iconViewFrame]; 
		[iv setImage: [self applicationIconImage]];
		[tmpIconWindow setContentView: iv];
		_app_icon_window = tmpIconWindow;
		RELEASE(iv);
	}
...
- (NSDictionary*) _notificationUserInfo
{
	...
	if (port == nil) {
		...
		if (path == nil) {
			userInfo = @{@"NSApplicationProcessIdentifier": processIdentifier};
		} else {
			userInfo = @{
				@"NSApplicationPath": path,
				@"NSApplicationProcessIdentifier": processIdentifier
			};
		}
	} else if (path == nil) {
		userInfo = @{
			@"NSApplicationName": port,
			@"NSApplicationProcessIdentifier": processIdentifier
		};
	} else {
		userInfo = @{
...
- (id) _targetForAction: (SEL)aSelector keyWindow: (NSWindow *)keyWindow mainWindow: (NSWindow *)mainWindow
{
	...
	sdc = [NSDocumentController sharedDocumentController];
	if ([[sdc documentClassNames] count] > 0 && [sdc respondsToSelector: aSelector]) {
		return sdc;
	}
...
- (void) _lastWindowClosed
{
	...
	} else if (NSInterfaceStyleForKey(@"NSMenuInterfaceStyle", nil) == NSWindows95InterfaceStyle) {
...
- (void) _windowWillClose: (NSNotification*) notification
{
	...
	if (_app_is_running) {
		...
		if (wasLast) {
			[self performSelector: @selector(_lastWindowClosed) withObject: nil afterDelay: 0.1];
		}
	}
}
...
- (NSMenu *) _dockMenu
{
	...
	for (i = j = 0, n = [windowsMenu numberOfItems]; i < n; i++) {
		...
		if ([[item target] isKindOfClass:[NSWindow class]] && sel_isEqual([item action], @selector(makeKeyAndOrderFront:))) {
			[[dockMenu insertItemWithTitle: [item title] action: @selector(makeKeyAndOrderFront:) keyEquivalent: @"" atIndex: j++] setTarget: [item target]];
		}
	}
	...
	if ([self isHidden]) {
		[dockMenu addItemWithTitle:_(@"Show") action:@selector(unhide:) keyEquivalent:@""];
	} else {
		[dockMenu addItemWithTitle:_(@"Hide") action:@selector(hide:) keyEquivalent:@""];
	}
	[dockMenu addItemWithTitle:_(@"Quit") action:@selector(terminate:) keyEquivalent:@""];
