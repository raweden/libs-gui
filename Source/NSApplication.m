...
_NSAppKitUncaughtExceptionHandler(NSException *exception)
{
     ...
     */
    if (GSCurrentContext() == nil || [[exception name] isEqual: NSWindowServerCommunicationException] || [[exception name] isEqual: GSWindowServerInternalException]) {
        // The following will raise again the exception using the base library exception handler
        [exception raise];
    }

    retVal = GSRunExceptionPanel([NSString stringWithFormat: _(@"Critical Error in %@"), [[NSProcessInfo processInfo] processName]], exception,
        _(@"Abort"), 
        nil,
#ifdef DEBUG
        _(@"Debug"));
    ...
    if ([theEvent clickCount] >= 2) {
        ...
        if ([NSApp isHidden] == NO) {
            ...
            while ((aWin = [iter nextObject])) {
                ...
                if ([aWin isVisible] == YES && [aWin isMiniaturized] == NO && aWin != [NSApp keyWindow] && aWin != [NSApp mainWindow] && aWin != [self window]  && ([aWin styleMask] & NSMiniWindowMask) == 0) {
...
- (void) finishLaunching
{
    ...
    [[[NSWorkspace sharedWorkspace] notificationCenter] addObserver: self selector: @selector(_workspaceNotification:) name: GSHideOtherApplicationsNotification object: nil];
    [[[NSWorkspace sharedWorkspace] notificationCenter] addObserver: self selector: @selector(_workspaceNotification:) name: GSUnhideAllApplicationsNotification object: nil];
...
- (void) _didFinishLaunching
{
    ...
    NS_DURING
    {
        [[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: NSWorkspaceDidLaunchApplicationNotification object: [NSWorkspace sharedWorkspace] userInfo: [self _notificationUserInfo]];
    }
    NS_HANDLER
    {
...
- (void) activateIgnoringOtherApps: (BOOL)flag
{
    ...
    if (_app_is_active == NO) {
        ...
        [[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: NSApplicationDidBecomeActiveNotification object: [NSWorkspace sharedWorkspace] userInfo: info];
    }
}
...
- (NSModalSession) beginModalSessionForWindow: (NSWindow*)theWindow
{
    ...
    theSession = (NSModalSession)NSZoneMalloc(NSDefaultMallocZone(), sizeof(struct _NSModalSession));
    ...
    switch (type) {
        ...
        case NSKeyDown:
        {
            ...
            /* Key equivalents must be looked up explicitly in the Services menu
                 ...
                 rationale. */
            if ([[self keyWindow] performKeyEquivalent: theEvent] == NO && [[self mainMenu] performKeyEquivalent: theEvent] == NO && [[self servicesMenu] performKeyEquivalent: theEvent] == NO)
            {
    ...
    if (event) {
        ...
        if (mode != NSEventTrackingRunLoopMode)
        {
            ...
            if ([NSCursor isHiddenUntilMouseMoves])
            {
                ...
                if ((type == NSLeftMouseDown) || (type == NSLeftMouseUp) || (type == NSOtherMouseDown) || (type == NSOtherMouseUp) || (type == NSRightMouseDown) || (type == NSRightMouseUp) || (type == NSMouseMoved))
                {
...
- (void) hide: (id)sender
{
    ...
    if (_app_is_hidden == NO)
    {
        if (![[NSUserDefaults standardUserDefaults] boolForKey: @"GSSuppressAppIcon"])
        {
            ...
            [[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: NSApplicationDidHideNotification object: [NSWorkspace sharedWorkspace] userInfo: info];
        } else {
...
- (void) unhideWithoutActivation
{
    if (_app_is_hidden == YES)
    {
        ...
        [[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: NSApplicationDidUnhideNotification object: [NSWorkspace sharedWorkspace] userInfo: info];
    }
}
...
- (void) presentError: (NSError*)error modalForWindow: (NSWindow*)window delegate: (id)delegate  didPresentSelector: (SEL)sel contextInfo: (void*)context
{
    ...
    [alert beginSheetModalForWindow: window modalDelegate: self didEndSelector: @selector(_didPresentError:returnCode:contextInfo:) contextInfo: wrapper];
}
...
- (void) _appIconInit
{
    ...
    _app_icon_window = [[NSIconWindow alloc] initWithContentRect: NSZeroRect styleMask: mask backing: NSBackingStoreRetained defer: NO screen: nil];
...
- (NSDictionary*) _notificationUserInfo
{
    ...
    if (port == nil)
    {
        if (path == nil)
        {
            userInfo = [NSDictionary dictionaryWithObjectsAndKeys: processIdentifier, @"NSApplicationProcessIdentifier", nil];
        } else {
            userInfo = [NSDictionary dictionaryWithObjectsAndKeys: path, @"NSApplicationPath", processIdentifier, @"NSApplicationProcessIdentifier", nil];
        }
    } else if (path == nil) {

        userInfo = [NSDictionary dictionaryWithObjectsAndKeys: port, @"NSApplicationName", processIdentifier, @"NSApplicationProcessIdentifier", nil];
    
    } else {
    
            userInfo = [NSDictionary dictionaryWithObjectsAndKeys: port, @"NSApplicationName", path, @"NSApplicationPath", processIdentifier, @"NSApplicationProcessIdentifier", nil];
    }
